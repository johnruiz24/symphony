...
test.describe("Flow 1: Create task via UI", () => {
  test("opens create dialog and submits a task", async ({ page }) => {
    // Find and click the "+ New Task" button in the header
    const addButton = page.getByRole("button", { name: "+ New Task" });
    await expect(addButton).toBeVisible({ timeout: 10_000 });
    await addButton.click();

    // Dialog should appear with "Create Task" heading
    const dialog = page.locator('.fixed.inset-0').filter({ has: page.getByRole("heading", { name: "Create Task" }) });
    await expect(dialog).toBeVisible();

    const uniqueTitle = `E2E Created Task ${Date.now()}`;
    ...
    // Submit via the form's submit button inside the dialog
  ...
  test("API-created task appears after page reload", async ({ page }) => {
    const uniqueTitle = `API-Side Task ${Date.now()}`;
    const result = await api.createTask(makeTask({ title: uniqueTitle }));
...
test.describe("Flow 2: Drag-drop task to column", () => {
  test("task can be dragged to a different status column", async ({ page }) => {
    // Create a task via API with unique name
    const uniqueTitle = `Drag Column Test ${Date.now()}`;
    const result = await api.createTask(makeTask({ title: uniqueTitle }));
    if (result.status !== 201) {
      test.skip();
      return;
    ...
    const taskCard = page.locator("h3").filter({ hasText: uniqueTitle }).locator("..");
...
test.describe("Flow 5: Search and filter", () => {
  ...
  test("priority filter narrows visible tasks", async ({ page }) => {
    const uniqueTitle = `HighPri Task ${Date.now()}`;
    await api.createTask(makeTask({ title: uniqueTitle, priority: "high" }));
